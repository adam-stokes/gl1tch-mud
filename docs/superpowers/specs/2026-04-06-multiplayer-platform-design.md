...
type Store interface {
    // Player state
    GetPlayerState(accountID, worldID string) (*PlayerState, error)
    SavePlayerState(accountID, worldID string, state *PlayerState) error

    // Inventory
    AddItem(accountID, worldID string, item Item) error
    RemoveItem(accountID, worldID string, itemID string) error
    GetInventory(accountID, worldID string) ([]Item, error)

    // NPC state
    GetNPCState(worldID, npcID string) (*NPCState, error)
    SetNPCState(worldID, npcID string, state *NPCState) error

    // ... mirrors existing db function signatures
}
```

Two implementations: `store.SQLiteStore` (wraps current code) and `store.PgStore` (new). World mode determines which is injected into command handlers.
...
| Package | Change |
|---------|--------|
| `internal/db/` | Extract interface into `internal/store/` |
| `internal/commands/` | Handler signature: `*sql.DB` → `store.Store` |
...
| Package | Purpose |
|---------|---------|
| `internal/store/` | Store interface + SQLite and Postgres implementations |
