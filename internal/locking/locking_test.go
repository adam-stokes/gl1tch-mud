...
func TestUnlockFunc(t *testing.T) {
	...
	if err := Unlock(gdb, "lock-1"); err != nil {
...
func TestPickSuccess(t *testing.T) {
	...
	found := false
	for i := 0; i < 20; i++ {
		res := Pick(gdb, "lock-easy", 1, 100)
		if res.Success {
			found = true
			if IsLocked(gdb, "lock-easy") {
...
func TestUnlockWithKeyHelper(t *testing.T) {
	...
	for _, keyID := range lockKeys {
		for _, itemID := range playerInv {
			if keyID == itemID {
				matched = true
				Unlock(gdb, "vault-door") //nolint:errcheck
			}
		}
	}
	if !matched {
