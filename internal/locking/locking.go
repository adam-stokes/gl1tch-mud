...
func IsLocked(db *sql.DB, lockID string) bool {
	var unlocked int
...
func Unlock(db *sql.DB, lockID string) error {
	_, err := db.Exec(
...
func Pick(db *sql.DB, lockID string, difficulty, pickingSkill int) PickResult {
...
func UnlockWithKey(db *sql.DB, lock *world.Lock, inventory []string) (bool, string) {
	for _, keyID := range lock.Keys {
		for _, itemID := range inventory {
			if itemID == keyID {
				if err := Unlock(db, lock.ID); err != nil {
